...
class HeartDataset():
    ...
    def __init__(self, data, cfg, mode):  
        ...
        D, H, W = cfg.patch_shape_before_crop
    ...
    def __getitem__(self, idx): 
        ...
        patches, contours = crop_images_and_contours([x,y, y_dist, y_refROIs], contours, config.patch_shape_before_crop, tuple(center)) 
        ...
        if mode == Modes.TRAINING: 
            ...
            # flip augmentation (no permute[rotate] sinze H/W != 1)
        ...
        y_stack_center_coords = y_centers_coords_.float().mean(dim=0)[1:] 
  
        return {   'x': x.detach(),   
                'y_voxels_before_crop': y.detach(),   
                'y_dist': y_dist.detach(),
                'y_center_before_crop': y_centers_coords,
                'y_stack_center_before_crop': y_stack_center_coords,
