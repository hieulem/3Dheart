...
class Stack:
    def __init__(self, slices, name, type, fill=False):
        ...
        self.shifts={}
        cls=1 #this is the oute layer of LV. best for aligning
        slice_ids = list(self.all_centers[cls].keys())
        for slice_id in slice_ids[1:]:
            self.shifts[slice_id] = self.all_centers[cls][slice_ids[0]]-self.all_centers[cls][slice_id]
    ...
    def compute_volumes(self, filled=False):
        ...
        for i, slice in enumerate(self.slices): 
            image = torch.from_numpy(slice.image).float().cuda()


            h_resolution, w_resolution, d_resolution = slice.dicom_resolution
            ...
            self.slices[i].image = image.cpu().numpy()

        self._draw_contours(fill=filled)  
        # mask_scar = self._to_stack('masks') > 0
        # mask_inner_wall = self._to_stack('masks') > 0
        # mask_outer_wall = self._to_stack('masks') > 0
         
        image_vol, masks_vol = self._to_stack() 
    ...
    def _to_stack(self):    
        ...
        for cls in self.classes:
            masks[cls] = np.zeros_like(images)
        for slice_id, slice in enumerate(self.slices): 
            for cls, mask in slice.masks.items(): 
                h, w = mask.shape  
                masks[cls][slice_id] = crop(mask, (H, W), (h//2, w//2)) 
        for cls in self.classes:
            masks[cls] = masks[cls]>0
        return images, masks
        ...
        for elem in root:  
            if elem.findtext('Vertices') is not None and int(elem.findtext('IndexType'))==cls: 
                ...
                for slice in slices: 
                    ...
                    if '034' in path: 
                        slice_z = 136.37 - slice.z
                    elif '008' in path: 
                        slice_z = -145.1661172853 + slice.z
                    elif '009' in path: 
                        slice_z = -104.76342 + slice.z 
                    elif '014' in path:
                        slice_z = -64.4966308197 + slice.z
                    elif '032' in path or '031' in path or '102' in path: 
                        break
                    else:
                        slice_z = slice.z
                ...
                if not found: 
                    ...
                    #     print(path)
                    #     printed = False
                    if '063' in path or '031' in path: 
                        slices[12 - int(index)].contours += [contour]  
                    elif '017' in path: 
                        slices[14 - int(index)].contours += [contour]  
                    elif '120' in path or '117' in path: 
                        slices[11 - int(index)].contours += [contour]  
                    elif '127' in path:  
                        slices[13 - int(index)].contours += [contour]  
                    else:
...
def resacle_z(x, scale_factor, mode='nearest'): 
    is_numpy = False 
    if scale_factor == 1:
        # print('o')
        return x
...
def load_stack(path, scale_factor=1):  
    ...
    for i, instance in enumerate(fs):
        # Load the corresponding SOP Instance dataset
        ds = instance.load()  
        if ds.SequenceName == '*tfl2d1_51':
             
            if 'examination_053' in path and ds.SeriesNumber != 1012003:
                continue

            elif 'examination_057' in path and ds.SeriesNumber != 167002:
                continue
