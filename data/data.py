from utils.utils_common import Modes, crop
...
class Normalize:
    ...
    def normalize_forward(self, vertices):
        assert len(vertices.shape) == 2 and len(self.shape.shape) == 2, "Inputs must be 2 dim"
        assert self.shape.shape[0] == 1, "first dim of shape should be length 1"
        
        if isinstance(vertices, torch.Tensor):
            return 2 * (vertices/(self.shape-1) - 0.5)
        else:
            return 2 * (vertices/(self.shape.numpy()-1) - 0.5)
        return vertices
    
    def normalize_backward(self, vertices):
        if isinstance(vertices, torch.Tensor):
            return 2 * (vertices/(self.shape-1) - 0.5)
        else:
            return 2 * (vertices/(self.shape.numpy()-1) - 0.5)     
...
def get_item(item, mode, config):
    ...
    # io.imsave('/cvlabdata2/home/wickrama/projects/annotator-mitk/interactive-annotation-mitk/server/outputs/overlay_{}.tif'.format(1), np.uint8(x_))
...
def sample_outer_surface_in_voxel(volume, inner=False): 
    if inner:
        # inner surface
...
def normalize_vertices2(vertices, shape):
    assert len(vertices.shape) == 2 and len(shape.shape) == 2, "Inputs must be 2 dim"
    assert shape.shape[0] == 1, "first dim of shape should be length 1"
    ...
    if isinstance(vertices, torch.Tensor):
        ...
        return 2 * (vertices/(shape.numpy()-1) - 0.5)
    # return 2*(vertices/(torch.max(shape)-1) - 0.5)
...
def voxel2mesh(volume, gap, shape):
    ...
    vertices_mc = normalize_vertices2(torch.from_numpy(vertices_mc).to(shape.device), shape)
    vertices_mc = torch.flip(vertices_mc, dims=[1]).float()  # convert z,y,x -> x, y, z
