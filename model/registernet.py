...
class RegisterNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        '''  Down layers '''
        down_layers = [UNetLayer(config.num_input_channels, first_layer_channels, config.ndims, kernel_size=[1,3,3], padding=[0,1,1])]
        ...
        self.fc_loc_laye1_size = feature_channel_count * 4 * 4
        # self.fc_loc_laye1_size = feature_channel_count * 5 * 4

        self.registration = lambda x: x[:, :feature_channel_count]
    ...
    def forward(self, data, iteration=0): 
        ...
        x_ = self.registration(x)
        x_ = x_.swapaxes(2,1)
        x_ = x_.reshape(x.shape[0], x.shape[2], -1)
        centers = self.fc_loc(x_)
    ...
    def loss(self, pred, data):
        ...
        y_voxels = data['y_voxels']
        y_voxels[y_voxels>1] = 1

        y_centers = data['y_center']
        ...
        mse_loss = 0.
        for k, v in y_centers.items():
            mse_loss += MSE_Loss(yhat_centers[:,k], v.float())
        ...
        log = {"loss_reg": loss.detach(), "ce_loss_reg": ce_loss.detach(), "mse_loss_reg": mse_loss.detach()}
        return loss, log
