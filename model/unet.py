...
class UNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.max_pool_asymetric = nn.MaxPool3d((1,2,2))
        ...
        for i in range(config.steps - 1, -1, -1):
            ...
            #     k_size = 2
            k_size = 2
            upconv = ConvTransposeLayer(in_channels=config.first_layer_channels   * 2**(i+1), out_channels=config.first_layer_channels * 2**i, kernel_size=k_size, stride=2)
            lyr = UNetLayer(config.first_layer_channels * 2**(i + 1), config.first_layer_channels * 2**i, config.ndims)
