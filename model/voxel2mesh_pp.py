...
class Voxel2Mesh(nn.Module):
    ...
    def __init__(self, config):
        ...
        print(config.graph_conv_layer_count)
        ...
        for i in range(config.steps+1):
            ...
            skip = NeighbourhoodSampling(config, self.skip_count[i], i, shape=config.patch_shape)
            skip_high_res = NeighbourhoodSampling(config, 1, i, shape=config.patch_shape)
            # lyr = Feature2VertexLayer(self.skip_count[i])
            if i == 0:
                grid_upconv_layer = None
                grid_unet_layer = None
                for k in range(config.lv_num_classes-1):
                    graph_unet_layers += [Features2Features(self.skip_count[i] + dim + high_res_skip, self.latent_features_coount[i], hidden_layer_count=config.graph_conv_layer_count)] # , graph_conv=GraphConv
                ...
                for k in range(config.lv_num_classes-1):
                    graph_unet_layers += [Features2Features(self.skip_count[i] + self.latent_features_coount[i-1] + dim + high_res_skip, self.latent_features_coount[i], hidden_layer_count=config.graph_conv_layer_count)] #, graph_conv=GraphConv if i < config.steps else GraphConvNoNeighbours

            for k in range(config.lv_num_classes-1):
                feature2vertex_layers += [Feature2VertexLayer(self.latent_features_coount[i], 3)] 
 

            up_std_conv_layers.append((skip, skip_high_res, grid_upconv_layer, grid_unet_layer))
        ...
        self.final_layer = ConvLayer(in_channels=config.first_layer_channels, out_channels=config.lv_num_classes, kernel_size=1)

        sphere_path='./spheres/icosahedron_{}.obj'.format(162)
        sphere_vertices, sphere_faces = read_obj(sphere_path)
        sphere_vertices = torch.from_numpy(sphere_vertices).to(self.device).float()
        self.sphere_vertices = sphere_vertices/torch.sqrt(torch.sum(sphere_vertices**2,dim=1)[:,None])[None]
        self.sphere_faces = torch.from_numpy(sphere_faces).to(self.device).long()[None]
    ...
    def forward(self, data):
        ...
        # resolution = data['resolution'].cuda()
        ...
        # vertices = (vertices/2 + 0.5) * (shape-1) * resolution
        ...
        for k in range(self.config.lv_num_classes-1):
            pred[k] = [[vertices.clone(), faces.clone(), None, None, sphere_vertices.clone()]]

 
        for i, ((skip_connection, skip_high_res_connection, grid_upconv_layer, grid_unet_layer), up_f2f_layers, up_f2v_layers, down_output, skip_amount, do_unpool) in enumerate(zip(self.up_std_conv_layers, self.up_f2f_layers, self.up_f2v_layers, down_outputs[::-1], self.skip_count, unpool_indices)):
            ...
            for k in range(self.config.lv_num_classes-1):
                ...
                # normalized_vertices = 2 * (vertices/((shape-1) * resolution) - 0.5)
                normalized_vertices = vertices 
                skipped_features = skip_connection(x[:, :(skip_amount)], normalized_vertices) 
                skipped_features_high_res = skip_high_res_connection(x_high_res, normalized_vertices)     
        
                latent_features = torch.cat([latent_features, skipped_features, skipped_features_high_res, vertices], dim=2) if latent_features is not None else torch.cat([skipped_features, skipped_features_high_res, vertices], dim=2)
 
                # print(f'{latent_features.device}, {A.device}, {D.device}, {vertices.device}, {faces.device}')
                ...
                if do_unpool[0] == 1:
                    ...
                    vertices, faces, latent_features, sphere_vertices = adoptive_unpool(vertices, faces_prev, sphere_vertices, latent_features, N_prev, self.device)
    ...
    def loss(self, pred, data): 
        ...
        log = {"loss_v2m": loss.detach(),
               "chamfer_loss_v2m": chamfer_loss.detach(), 
               "ce_loss_v2m": ce_loss.detach(),
               "normal_consistency_loss_v2m": normal_consistency_loss.detach(),
               "edge_loss_v2m": edge_loss.detach(),
               "laplacian_loss_v2m": laplacian_loss.detach()}
        return loss, log
