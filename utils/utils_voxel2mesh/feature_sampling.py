...
class NeighbourhoodSampling(nn.Module):
    ...
    def __init__(self, config, features_count, step, shape=None):
        super(NeighbourhoodSampling, self).__init__()
        device = config.device
        if shape is None:
            D, H, W = config.config.patch_shape
        else:
            D, H, W = shape

        self.shape = torch.tensor([W, H, D]).to(device).float()
        ...
        self.sum_neighbourhood = nn.Conv2d(features_count, features_count, kernel_size=(1, 27), padding=0).to(device)
