...
def adoptive_unpool(vertices, faces_prev, sphere_vertices, latent_features, N_prev, device):
    ...
    edge_combinations_3 = torch.tensor(list(combinations(range(3), 2))).to(device)
    ...
    faces = torch.from_numpy(hull.simplices).long().to(device)[None] 
